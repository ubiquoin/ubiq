...
{
    // Ubiquoin: check prefix
    if(uri.scheme() != QString("Ubiquoin"))
        return false;
...
{
    // Convert Ubiquoin:// to Ubiquoin:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("Ubiquoin://"))
    {
        uri.replace(0, 12, "Ubiquoin:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Ubiquoin.lnk";
}
...
{
    return GetAutostartDir() / "Ubiquoin.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Ubiquoin-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  Ubiquoin-qt [" + tr("command-line options") + "]                     " + "\n";
